...
// Mutate calls f(ctx, m).
func (f LangFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.LangMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.LangMutation", m)
	}
	return f(ctx, mv)
}
